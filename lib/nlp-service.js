module.exports = function(secretsManager) {
  ...
  class AylienApi {
    ...
    annotateText({ url=undefined, text=undefined, best_image=false, language="auto" } = {}) {
      var requestParams = {
        method: "GET",
      ...
      console.log('request params for api', requestParams);
...
//      })
  ...
  class NlpService { 
    ...
    categorizeText({ text=undefined } = {}) {
      return this._googleApi.categorizeText({ text: text });
    }
    
    annotateText({ text=undefined } = {}) {
      var googlePromise = this._googleApi.annotateText({ text: text });
      var ibmPromise = this._aylienApi.annotateText({ text: text});
